...
void main() {
  ...
  test('create writes the row to Drift and enqueues an outbox upsert', () async {
  ...
  test(
    'undoCreate before the create is pushed removes the outbox entry '
    'entirely — no delete is ever sent to the server',
    () async {
      final id = await repo.create(
        householdId: 'h1',
        userId: 'u1',
  ...
  test(
    'undoCreate after the create has already been pushed enqueues a real '
    'delete so the undo still propagates',
    () async {
      final id = await repo.create(
        householdId: 'h1',
        userId: 'u1',
  ...
  test('hasPossibleDuplicate finds a same amount/category row within 2 minutes', () async {
    await repo.create(
      householdId: 'h1',
      userId: 'u1',
      amountPaise: 5000,
