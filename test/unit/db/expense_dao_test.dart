...
void main() {
  ...
  test(
    'getFiltered returns every matching row unpaginated, for the Export screen (T-12.1)',
    () async {
      final now = DateTime.utc(2026, 9, 1);
      Future<void> insert(String id, String userId, String? categoryId, DateTime spentOn) =>
  ...
  group('hasAnyOn', () {
    ...
    test('false for a different date, another household, or a soft-deleted row', () async {
      final now = DateTime.utc(2026, 9, 6);
      await db.expenseDao.upsert(
        ExpensesCompanion.insert(
          id: 'e1',
