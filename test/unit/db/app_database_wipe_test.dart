...
void main() {
  ...
  test('wipeAll empties every table (spec T-3.6, sign-out privacy wipe)', () async {
    final now = DateTime.utc(2026, 9, 4);

    await db.profileDao.upsert(
      ProfilesCompanion.insert(id: 'u1', householdId: 'h1', displayName: 'Vineet', createdAt: now, updatedAt: now),
    );
    await db.categoryDao.upsert(
      CategoriesCompanion.insert(id: 'c1', householdId: 'h1', name: 'Food', createdAt: now, updatedAt: now),
    );
