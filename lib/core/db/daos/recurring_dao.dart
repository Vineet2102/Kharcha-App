...
class RecurringDao extends DatabaseAccessor<AppDatabase> with _$RecurringDaoMixin {
  ...
  Future<void> upsert(RecurringRulesCompanion entry) => into(recurringRules).insertOnConflictUpdate(entry);

  Future<void> softDelete(String id, DateTime now) => (update(recurringRules)..where((t) => t.id.equals(id))).write(
  ...
  Future<void> markSynced(String id) => (update(recurringRules)..where((t) => t.id.equals(id))).write(
        const RecurringRulesCompanion(isDirty: Value(false), syncStatus: Value('synced')),
      );

  Future<int> hardDelete(String id) => (delete(recurringRules)..where((t) => t.id.equals(id))).go();
}
